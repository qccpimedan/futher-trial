...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('pemeriksaan_produk_cooking_mixer_fla', function (Blueprint $table) {
            ...
            $table->foreign('qc_approved_by', 'fk_ppcmf_qc_approved_by')->references('id')->on('users')->onDelete('set null');
            $table->foreign('produksi_approved_by', 'fk_ppcmf_produksi_approved_by')->references('id')->on('users')->onDelete('set null');
            $table->foreign('spv_approved_by', 'fk_ppcmf_spv_approved_by')->references('id')->on('users')->onDelete('set null');
        });
    }
    ...
    {
        Schema::table('pemeriksaan_produk_cooking_mixer_fla', function (Blueprint $table) {
            // Drop foreign keys first using custom constraint names
            $table->dropForeign('fk_ppcmf_qc_approved_by');
            $table->dropForeign('fk_ppcmf_produksi_approved_by');
            $table->dropForeign('fk_ppcmf_spv_approved_by');
