...
class ReboxImport implements WithMultipleSheets
{
    protected $user;
    ...
    {
        return [
            'Template' => new ReboxTemplateSheetImport($this),
        ];
    }
...
class ReboxTemplateSheetImport implements ToCollection, WithStartRow, WithCalculatedFormulas
{
    protected ReboxImport $parent;

    public function __construct(ReboxImport $parent)
    {
        $this->parent = $parent;
    }
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $i => $row) {
            ...
            $jamVal = $row[2] ?? null;
            $namaProduk = trim((string) ($row[3] ?? ''));
            $kodeProduksi = trim((string) ($row[4] ?? ''));
            $bestBeforeVal = $row[5] ?? null;
            $isiJumlah = trim((string) ($row[6] ?? ''));
            $labelisasi = trim((string) ($row[7] ?? ''));
            ...
            if ($namaProduk === '') {
                $reasons[] = 'Nama Produk kosong (cek kolom D)';
            }
            if ($kodeProduksi === '') {
            ...
            if ($isiJumlah === '') {
                $reasons[] = 'Isi/Jumlah kosong (cek kolom G)';
            }
            if ($labelisasi === '') {
                $reasons[] = 'Labelisasi kosong (cek kolom H)';
            }
