...
class SeasoningImport implements WithMultipleSheets
{
    protected $user;
    ...
    {
        return [
            'Template' => new SeasoningTemplateSheetImport($this),
        ];
    }
...
class SeasoningTemplateSheetImport implements ToCollection, WithStartRow, WithCalculatedFormulas
{
    protected SeasoningImport $parent;

    public function __construct(SeasoningImport $parent)
    {
        $this->parent = $parent;
    }
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $i => $row) {
            ...
            $jamVal = $row[2] ?? null;
            $namaRm = trim((string) ($row[3] ?? ''));
            $kodeProduksi = trim((string) ($row[4] ?? ''));
            $berat = trim((string) ($row[5] ?? ''));
            $sensori = trim((string) ($row[6] ?? ''));
            $kemasan = trim((string) ($row[7] ?? ''));
            $keterangan = trim((string) ($row[8] ?? ''));

            if ($shiftVal === '' && $namaRm === '' && $kodeProduksi === '') {
                continue;
            }
            ...
            if ($jamVal !== null && $jamVal !== '') {
                if (is_numeric($jamVal)) {
                    try {
                        $jam = ExcelDate::excelToDateTimeObject($jamVal)->format('H:i:s');
                    } catch (\Exception $e) {
                        $jam = null;
                    }
                } else {
                    try {
                        $jam = Carbon::parse($jamVal)->format('H:i:s');
                    } catch (\Exception $e) {
                        $jam = null;
                    }
                }
            ...
            if ($namaRm === '') {
                $reasons[] = 'Nama Seasoning kosong (cek kolom D)';
            }
            ...
            if ($berat === '') {
                $reasons[] = 'Berat kosong (cek kolom F)';
            }

            if ($sensori === '') {
                $reasons[] = 'Sensori kosong (cek kolom G)';
            }

            if ($kemasan === '') {
                $reasons[] = 'Kemasan kosong (cek kolom H)';
            }
        ...
        if (!empty($insertData)) {
            // Using Chunk insert to follow pattern or direct insert if not too many
            foreach (array_chunk($insertData, 100) as $chunk) {
                Seasoning::insert($chunk);
            }
