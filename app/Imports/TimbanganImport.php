...
class TimbanganImport implements WithMultipleSheets
{
    protected $user;
    ...
    {
        return [
            'Template' => new TimbanganTemplateSheetImport($this),
        ];
    }
...
class TimbanganTemplateSheetImport implements ToCollection, WithStartRow, WithCalculatedFormulas
{
    protected TimbanganImport $parent;

    public function __construct(TimbanganImport $parent)
    {
        $this->parent = $parent;
    }
    ...
    public function collection(Collection $rows)
    {
        ...
        $timbanganData = DataTimbangan::where('id_plan', $user->id_plan)
            ->get(['nama_timbangan', 'kode_timbangan']);
        
        $timbanganMap = $timbanganData->pluck('kode_timbangan', 'nama_timbangan')->toArray();
        ...
        foreach ($rows as $i => $row) {
            ...
            $jamVal = $row[2] ?? null;
            $namaTimbangan = trim((string) ($row[3] ?? ''));
            $hasilPengecekan = strtolower(trim((string) ($row[4] ?? '')));
            $gram = trim((string) ($row[5] ?? ''));
            $hasil500 = trim((string) ($row[6] ?? ''));
            $hasil1000 = trim((string) ($row[7] ?? ''));

            if ($shiftVal === '' && $namaTimbangan === '' && $hasilPengecekan === '') {
                continue;
            }
            ...
            // Timbangan Mapping
            $kodeTimbangan = $timbanganMap[$namaTimbangan] ?? null;
            if (!$kodeTimbangan) {
                $reasons[] = "Nama Timbangan tidak valid: '{$namaTimbangan}'";
            }
            ...
            if ($gram !== '' && !isset($gramOptions[$gram])) {
                $reasons[] = "Gram tidak valid: '{$gram}' (Gunakan: 500 / 1000)";
            }
            ...
            $insertData[] = [
                ...
                'jam' => $jam,
                'jenis' => $namaTimbangan,
                'kode_timbangan' => $kodeTimbangan,
                'hasil_pengecekan' => $hasilPengecekan,
                'gram' => $gram ?: '500',
                'hasil_verifikasi_500' => $hasil500 ?: null,
                'hasil_verifikasi_1000' => $hasil1000 ?: null,
                'created_at' => now(),
                'updated_at' => now(),
            ];
        }
