...
{{-- filepath: resources/views/qc-sistem/seasoning/_table.blade.php --}}

@if(isset($seasonings) && count($seasonings))
<div class="table-responsive">
    <table style="white-space: nowrap;" class="text-center table table-bordered table-striped table-hover">
        ...
        <tbody>
            @foreach($seasonings as $item)
            <tr>
                <td>{{ $seasonings->firstItem() + $loop->index }}</td>
                <td>
                ...
                <td class="text-center">
                    <div class="btn-vertical">
                        ...
                        <div class="mb-1">
                            @if(auth()->user()->hasPermissionTo('edit-seasoning'))
                            <a href="{{ route('seasoning.edit', $item->uuid) }}" 
                               class="btn btn-sm btn-warning" 
                               title="Edit Data">
                                <i class="fas fa-edit"></i>
                            </a>
                            @endif
                            @if(auth()->user()->hasPermissionTo('view-seasoning'))
                            <a href="{{ route('seasoning.logs', $item->uuid) }}" 
                               class="btn btn-sm btn-info" 
                               title="Lihat Log">
                                <i class="fas fa-history"></i>
                            </a>
                            @endif
                            @if(auth()->user()->hasPermissionTo('delete-seasoning'))
                            <form action="{{ route('seasoning.destroy', $item->uuid) }}" 
                                  method="POST" 
                                  class="d-inline">
                                @csrf
                                @method('DELETE')
    ...
    <div class="d-flex justify-content-center mt-4">
        {{ $seasonings->appends(request()->query())->links('pagination::bootstrap-4') }}
    </div>
</div>
...
$(document).ready(function() {
    $('.approve-btn').click(function() {
        ...
        if (confirm(`Apakah Anda yakin ingin menyetujui data ini sebagai ${typeNames[type]}?`)) {
            ...
            $.ajax({
                url: '{{ route("seasoning.approve", ":id") }}'.replace(':id', id),
                method: 'POST',
                data: {
...
<div class="alert alert-info">
    <i class="fas fa-info-circle"></i> Tidak ada data seasoning yang tersedia.
</div>
@endif