...
@section('title', 'Riwayat Perubahan Data - Berat Produk Box')
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Riwayat Perubahan Box</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-3">
                                    ...
                                    <span class="badge badge-secondary">{{ \Carbon\Carbon::parse($beratProdukBox->tanggal)->format('d/m/Y H:i') }}</span>
                                </div>
                                <div class="col-md-3">
                                    <strong>Plan:</strong><br>
                                    <span class="badge badge-info">{{ $beratProdukBox->plan->nama_plan ?? '-' }}</span>
                                </div>
                                <div class="col-md-3">
                                    <strong>Shift:</strong><br>
                                    <span class="badge badge-primary">{{ $beratProdukBox->shift->shift ?? '-' }}</span>
                                </div>
                                <div class="col-md-3">
                                    <strong>Data Box:</strong><br>
                                    <span class="badge badge-success">{{ $beratProdukBox->data_box->std_box ?? '-' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('berat-produk.index') }}" class="btn btn-secondary btn-sm">
                        ...
                        <div class="card-body">
                            @php
                                $logs = \App\Models\BeratProdukBoxLog::where('berat_produk_box_uuid', $beratProdukBox->uuid)
                                    ->with('user')
                                                ...
                                                <td>
                                                    @if($log->field_yang_diubah && count($log->field_yang_diubah) > 0)
                                                        @foreach($log->field_yang_diubah as $field)
                                                            <span class="text-primary">{{ $log->getNamaField($field) }}</span>
                                                        ...
                                                        @if($log->nilai_lama && $log->nilai_baru)
                                                            @foreach($log->field_yang_diubah as $field)
                                                                @php
                                                                    ...
                                                                    $namaField = $log->getNamaField($field);
                                                                @endphp
                                                                <div class="mb-1">
