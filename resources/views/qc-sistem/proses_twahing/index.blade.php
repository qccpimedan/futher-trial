...
@section('title', 'Pemeriksaan Proses Thawing')
    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Pemeriksaan Proses Thawing</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="#">Home</a></li>
                            <li class="breadcrumb-item active">Proses Thawing</li>
                        </ol>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Data Pemeriksaan Proses Thawing</h3>
                                <div class="card-tools">
                                    @if(auth()->user()->hasPermissionTo('create-proses-twahing'))
                                    <a href="{{ route('proses-twahing.create') }}" class="btn btn-primary btn-sm">
                                ...
                                <div class="table-responsive text-center">
                                    ...
                                    <div class="row mb-3 mt-3">
                                        <div class="col-md-4 offset-md-8">
                                            <form action="{{ route('proses-twahing.index') }}" method="GET">
                                                <div class="input-group">
                                                    <input type="text" name="search" class="form-control" placeholder="Cari Nama RM atau Tanggal" value="{{ request('search') }}">
                                                    <div class="input-group-append">
                                                        <button class="btn btn-outline-secondary" type="submit">
                                                            <i class="fas fa-search"></i> Cari
                                                        </button>
                                                        @if(request('search'))
                                                            <a href="{{ route('proses-twahing.index') }}" class="btn btn-outline-danger" title="Clear Search">
                                        ...
                                        <tbody>
                                            @forelse($items as $index => $item)
                                                <tr>
                                                    ...
                                                    <td>{{ $item->kondisi_kemasan_rm ?? '-' }}</td>
                                                    <td>{{ $item->plan->nama_plan ?? '-' }}</td>
                                                    <td>{{ $item->user->name ?? '-' }}</td>
                                                    <td>
                                                        <div class="btn-vertical">
                                                            <div class="mb-1">
                                                                <a href="{{ route('proses-twahing.show', $item->uuid) }}" class="btn btn-info btn-sm">
...
@if(auth()->user()->hasPermissionTo('edit-proses-twahing'))
                                                                ...
                                                                @endif
@if(auth()->user()->hasPermissionTo('delete-proses-twahing'))
<form action="{{ route('proses-twahing.destroy', $item->uuid) }}" method="POST" style="display:inline;">
                                                                    @csrf
                                                                    @method('DELETE')
                                                            ...
                                                            @php
                                                                ...
                                                                $hasApprovalColumns = Schema::hasColumn('proses_twahing', 'approved_by_qc');
                                                            @endphp
...
$(document).ready(function() {
    $('.approve-btn').click(function() {
        ...
        if (confirm(`Apakah Anda yakin ingin menyetujui data ini sebagai ${typeNames[type]}?`)) {
            ...
            $.ajax({
                url: '{{ route("proses-twahing.approve", ":id") }}'.replace(':id', id),
                method: 'POST',
                data: {
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exportPdfModalLabel">Cetak PDF Proses Thawing Berdasarkan Filter</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="bulkExportForm" action="{{ route('proses-twahing.bulk-export-pdf') }}" method="POST" data-bulk-form="true" target="_blank">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="bulkKodeForm" name="kode_form"  value="QF 33/00" placeholder="Masukkan kode form" readonly>
                    </div>
                </form>
            </div>
