...
<head>
    <meta charset="utf-8">
    <title>Verifikasi Proses Produksi - Kontrol Sanitasi</title>
    <style>
        body {
...
<body>
    ...
    <div class="title">VERIFIKASI KONTROL SANITASI</div>
    ...
    @if($data->count() > 0)
        ...
        <div class="form-info">
            <table>
                <tr>
                    <td>Hari / Tanggal</td>
                    <td colspan="3">: {{ $firstItem->tanggal ? (is_string($firstItem->tanggal) ? \Carbon\Carbon::parse($firstItem->tanggal)->format('l, d-m-Y') : $firstItem->tanggal->format('l, d-m-Y')) : '-' }}</td>
                </tr>
                <tr>
                    <td>Shift</td>
        ...
        <table class="main-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 15%;">Suhu Air</th>
                    <th style="width: 20%;">Kadar Klorin Food Basin</th>
                    <th style="width: 20%;">Kadar Klorin Hand Basin</th>
                    <th style="width: 30%;">Hasil Verifikasi</th>
                </tr>
            </thead>
            <tbody>
                @foreach($data as $index => $item)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $item->jam ? (is_string($item->jam) ? \Carbon\Carbon::parse($item->jam)->format('H:i') : $item->jam->format('H:i')) : '-' }}</td>
                        <td>{{ $item->suhu_air }}°C</td>
                        <td>{{ $item->kadar_klorin_food_basin }}</td>
                        <td>{{ $item->kadar_klorin_hand_basin }}</td>
 ...
 <div style="width: 100%; text-align: right; margin-top: 10px;">
        ...
        @php
            ...
            if($qcApprover) {
                $qcApprovalDate = '';
                foreach($data as $item) { if($item->approved_by_qc && $item->qc_approved_at) { $qcApprovalDate = is_string($item->qc_approved_at) ? \Carbon\Carbon::parse($item->qc_approved_at)->format('Y-m-d') : $item->qc_approved_at->format('Y-m-d'); break; } }
            ...
            if($spvApprover) {
                $spvApprovalDate = '';
                foreach($data as $item) { if($item->approved_by_spv && $item->spv_approved_at) { $spvApprovalDate = is_string($item->spv_approved_at) ? \Carbon\Carbon::parse($item->spv_approved_at)->format('Y-m-d') : $item->spv_approved_at->format('Y-m-d'); break; } }
            ...
            if($produksiApprover) {
                ...
                foreach($data as $item) { if($item->approved_by_produksi && $item->produksi_approved_at) { $produksiApprovalDate = is_string($item->produksi_approved_at) ? \Carbon\Carbon::parse($item->produksi_approved_at)->format('Y-m-d') : $item->produksi_approved_at->format('Y-m-d'); break; } }
