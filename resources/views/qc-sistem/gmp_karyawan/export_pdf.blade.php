...
<head>
    <meta charset="utf-8">
    <title>Verifikasi Proses Produksi - GMP Karyawan</title>
    <style>
        body {
...
<body>
    ...
    <div class="title">Kebersihan Karyawan</div>
    ...
    @if($data->count() > 0)
        ...
        <table class="main-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 20%;">Temuan Ketidaksesuaian</th>
                    <th style="width: 20%;">Keterangan</th>
                    <th style="width: 20%;">Tindakan Koreksi</th>
            ...
            <tbody>
                @foreach($data as $index => $item)
                    <tr>
                        ...
                        <td>{{ $item->temuan_ketidaksesuaian_label ?? $item->temuan_ketidaksesuaian ?? '-' }}</td>
                        ...
                        <td>
                            ...
                            @endphp
                            {{ $verifikasi === 'ok' ? 'OK' : ($verifikasi === 'tidak_ok' ? 'Tidak OK' : '-') }}
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        @php
            ...
            foreach($data as $item) {
                if($item->approved_by_qc && $item->approved_by_qc_user_id) {
                    $qcApprover = \App\Models\User::find($item->approved_by_qc_user_id);
                    if($qcApprover) { $qcApproverName = $qcApprover->name; break; }
                }
            }
            $spvApprover = null; $spvApproverName = 'SPV';
            foreach($data as $item) {
                if($item->approved_by_spv && $item->approved_by_spv_user_id) {
                    $spvApprover = \App\Models\User::find($item->approved_by_spv_user_id);
                    if($spvApprover) { $spvApproverName = $spvApprover->name; break; }
                }
            }
            $produksiApprover = null; $produksiApproverName = 'FM/FL PRODUKSI';
            foreach($data as $item) {
                if($item->approved_by_produksi && $item->approved_by_produksi_user_id) {
                    $produksiApprover = \App\Models\User::find($item->approved_by_produksi_user_id);
            ...
            if($qcApprover) {
                $qcApprovalDate = '';
                foreach($data as $item) { if($item->approved_by_qc && $item->approved_by_qc_at) { $qcApprovalDate = \Carbon\Carbon::parse($item->approved_by_qc_at)->format('Y-m-d'); break; } }
            ...
            if($spvApprover) {
                $spvApprovalDate = '';
                foreach($data as $item) { if($item->approved_by_spv && $item->approved_by_spv_at) { $spvApprovalDate = \Carbon\Carbon::parse($item->approved_by_spv_at)->format('Y-m-d'); break; } }
            ...
            if($produksiApprover) {
                ...
                foreach($data as $item) { if($item->approved_by_produksi && $item->approved_by_produksi_at) { $produksiApprovalDate = \Carbon\Carbon::parse($item->approved_by_produksi_at)->format('Y-m-d'); break; } }
