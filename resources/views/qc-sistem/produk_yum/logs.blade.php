...
@section('title', 'Riwayat Perubahan Data - KPI Produk YUM')
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <span class="badge badge-info">{{ $produkYum->produk->nama_produk ?? '-' }}</span>
                                </div>
                                <div class="col-md-4">
                                    <strong>Tanggal:</strong><br>
                                    <span class="badge badge-secondary">{{ $produkYum->tanggal ? $produkYum->tanggal->format('d/m/Y') : '-' }}</span>
                                </div>
                                <div class="col-md-4">
                                    <strong>Shift:</strong><br>
                                    <span class="badge badge-primary">{{ $produkYum->shift->shift ?? '-' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('produk-yum.index') }}" class="btn btn-secondary btn-sm">
                                    ...
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge badge-info">{{ ucfirst($log->user->role ?? 'Unknown') }}</span>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    @if($log->field_yang_diubah && count($log->field_yang_diubah) > 0)
                                                        @foreach($log->field_yang_diubah as $field)
                                                            <span class="text-primary">{{ $log->getFieldNames()[$field] ?? $field }}</span>
                                                        ...
                                                        @if($log->nilai_lama && $log->nilai_baru && $log->field_yang_diubah)
                                                            @foreach($log->field_yang_diubah as $index => $field)
                                                                @php
                                                                    ...
                                                                    $namaField = $log->getFieldNames()[$field] ?? $field;
