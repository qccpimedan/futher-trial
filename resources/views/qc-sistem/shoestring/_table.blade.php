...
{{-- filepath: resources/views/qc-sistem/shoestring/_table.blade.php --}}

@if(isset($shoestrings) && count($shoestrings))
<div class="table-responsive">
    <table style="white-space: nowrap;" class="text-center table table-bordered table-striped table-hover">
        ...
        <tbody>
            @foreach($shoestrings as $item)
            <tr>
            <td>{{ $shoestrings->firstItem() + $loop->index }}</td>
                <td>
                ...
                <td class="font-weight-medium">{{ $item->nama_produsen ?? '-' }}</td>
                <td>{{ $item->kode_produksi ?? '-' }}</td>
                <td>{{ $item->createdBy->name ?? '-' }}</td>
                <!-- <td class="text-center">
                    {{ $item->best_before ? \Carbon\Carbon::parse($item->best_before)->format('d-m-Y') : '-' }}
                </td> -->
                <!-- <td class="text-center">{{ $item->sampling_defect }}</td> -->
                <!-- <td>
                    @if($item->catatan)
                        <span class="text-truncate d-inline-block" style="max-width: 150px;" title="{{ $item->catatan }}">
                            {{ $item->catatan }}
                        </span>
                    @else
                ...
                <td class="text-center">
                    <div class="btn-vertical">
                        ...
                        <div class="mb-1">
                            <!--
@if(auth()->user()->hasPermissionTo('edit-shoestring')) <a href="{{ route('shoestring.edit', $item->uuid) }}" 
                               class="btn btn-sm btn-warning" 
                               title="Edit Data">
                                <i class="fas fa-edit"></i>
                            </a>
                            @endif
@if(auth()->user()->hasPermissionTo('view-shoestring'))
<a href="{{ route('shoestring.logs', $item->uuid) }}" 
                               class="btn btn-sm btn-info" 
                               title="Lihat Log">
                                <i class="fas fa-history"></i>
                            </a>
                            @endif
@if(auth()->user()->hasPermissionTo('delete-shoestring'))
<form action="{{ route('shoestring.destroy', $item->uuid) }}" 
                                  method="POST" 
                                  class="d-inline">
                                @csrf
                                @method('DELETE')
...
-->
                            <x-action-buttons :item="$item" route-prefix="shoestring" :show-view="true"/>
    ...
    <div class="d-flex justify-content-center mt-4">
        {{ $shoestrings->appends(request()->query())->links('pagination::bootstrap-4') }}
    </div>
</div>
...
$(document).ready(function() {
    $('.approve-btn').click(function() {
        ...
        if (confirm(`Apakah Anda yakin ingin menyetujui data ini sebagai ${typeNames[type]}?`)) {
            ...
            $.ajax({
                url: '{{ route("shoestring.approve", ":id") }}'.replace(':id', id),
                method: 'POST',
                data: {
...
<div class="alert alert-info">
    <i class="fas fa-info-circle"></i> Tidak ada data shoestring yang tersedia.
</div>
@endif