...
@section('title', 'Riwayat Perubahan Data - Produk Forming')
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-3">
                                    ...
                                    <span class="badge badge-secondary">{{ \Carbon\Carbon::parse($produkForming->tanggal)->format('d/m/Y H:i') }}</span>
                                </div>
                                <div class="col-md-3">
                                    <strong>Plan:</strong><br>
                                    <span class="badge badge-info">{{ $produkForming->plan->nama_plan ?? '-' }}</span>
                                </div>
                                <div class="col-md-3">
                                    <strong>Shift:</strong><br>
                                    <span class="badge badge-primary">{{ $produkForming->shift->shift ?? '-' }}</span>
                                </div>
                                <div class="col-md-3">
                                    <strong>Produk:</strong><br>
                                    <span class="badge badge-success">{{ $produkForming->produk->nama_produk ?? '-' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('produk-forming.index') }}" class="btn btn-secondary btn-sm">
                        ...
                        <div class="card-body">
                            @php
                                $logs = \App\Models\ProdukFormingLog::where('produk_forming_uuid', $produkForming->uuid)
                                    ->with('user')
                                                ...
                                                <td>
                                                    @if($log->field_yang_diubah && count($log->field_yang_diubah) > 0)
                                                        @foreach($log->field_yang_diubah as $field)
                                                            @php
                                                                $fieldNames = \App\Models\ProdukFormingLog::getFieldNames();
