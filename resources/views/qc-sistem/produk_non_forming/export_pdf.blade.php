...
<head>
    <meta charset="utf-8">
    <title>Verifikasi Proses Produksi - Produk Non Forming</title>
    <style>
        body {
...
<body>
    ...
    <div class="title">VERIFIKASI PERGANTIAN PRODUK NON FORMING</div>
    ...
    @if(count($data) > 0)
        @php
            // Helper function to convert verifikasi symbols
            function formatVerifikasi($verifikasi) {
                if ($verifikasi === '✓') {
                    return 'OK';
                } elseif ($verifikasi === '✗') {
                    return 'NOT OK';
        ...
        <table class="main-table">
            ...
            <tbody>
                            ...
                            @if($bahan_baku_count < 5)
                                ...
                                <tr>
                                    ...
                                    @if($bahan_baku_count == 1)
                                        ...
                                        <td rowspan="5" style="vertical-align: middle; border: 1px solid #000; text-align: center; font-size: 7px;">{{ formatVerifikasi($item->verifikasi) }}</td>
                                    @endif
                                </tr>
                            @endif
                        @endforeach
                ...
                @for($i = $bahan_baku_count + 1; $i <= 5; $i++)
                    <tr>
                        ...
                        @if($bahan_baku_count == 0 && $i == 1)
                            ...
                            <td rowspan="5" style="vertical-align: middle; border: 1px solid #000; text-align: center; font-size: 7px;">{{ formatVerifikasi($data->first()->verifikasi) }}</td>
                        @endif
                    </tr>
                @endfor
                            ...
                            @if($bahan_penunjang_count < 12)
                                ...
                                <tr>
                                    ...
                                    @if($bahan_penunjang_count == 1)
                                        ...
                                        <td rowspan="12" style="vertical-align: middle; border: 1px solid #000; text-align: center; font-size: 7px;">{{ formatVerifikasi($item->verifikasi) }}</td>
                                    @endif
                                </tr>
                            @endif
                        @endforeach
                ...
                @for($i = $bahan_penunjang_count + 1; $i <= 12; $i++)
                    <tr>
                        ...
                        @if($bahan_penunjang_count == 0 && $i == 1)
                            ...
                            <td rowspan="12" style="vertical-align: middle; border: 1px solid #000; text-align: center; font-size: 7px;">{{ formatVerifikasi($data->first()->verifikasi) }}</td>
                        @endif
                    </tr>
                @endfor
                ...
                @foreach($kemasan_items as $index => $kemasan)
                    <tr>
                        ...
                        @if($index == 0)
                            ...
                            <td rowspan="4" style="vertical-align: middle; border: 1px solid #000; text-align: center; font-size: 7px;">{{ formatVerifikasi($data->first()->verifikasi) }}</td>
                        @endif
                    </tr>
                @endforeach
                ...
                @php 
                    $mesin_items = [
                        ['name' => 'TUMBLER', 'value' => $data->first()->tumbler ?? null],
                        ['name' => 'FRAYER', 'value' => $data->first()->frayer ?? null],
                        ['name' => 'HICOOK', 'value' => $data->first()->hicook ?? null],
                        ['name' => 'IQF ADVANCE 1', 'value' => $data->first()->iqf_advance_1 ?? null],
                        ['name' => 'IQF ADVANCE 2', 'value' => $data->first()->iqf_advance_2 ?? null],
                        ['name' => 'KERANJANG', 'value' => $data->first()->keranjang ?? null],
                        ['name' => 'PALET', 'value' => $data->first()->palet ?? null],
                        ...
                        ['name' => 'KARTON SEALER', 'value' => $data->first()->karton_sealer ?? null]
                    ];
                @endphp
                @foreach($mesin_items as $index => $mesin)
                    <tr>
                        ...
                        <td style="text-align: center; font-size: 5px;">{{ formatVerifikasi($data->first()->verifikasi) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    @else
        <div style="text-align: center; padding: 40px; border: 2px dashed #ccc; margin: 20px 0; background-color: #f9f9f9;">
            ...
            <p style="color: #666; margin-bottom: 10px; font-size: 12px;">
                Tidak ada data Produk Non Forming yang sesuai dengan filter yang dipilih:
            </p>
