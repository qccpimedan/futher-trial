...
@section('title', 'Riwayat Perubahan Data - Produk Non Forming')
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-3">
                                    ...
                                    <span class="badge badge-secondary">{{ \Carbon\Carbon::parse($produkNonForming->tanggal)->format('d/m/Y H:i') }}</span>
                                </div>
                                <div class="col-md-3">
                                    <strong>Plan:</strong><br>
                                    <span class="badge badge-info">{{ $produkNonForming->plan->nama_plan ?? '-' }}</span>
                                </div>
                                <div class="col-md-3">
                                    <strong>Shift:</strong><br>
                                    <span class="badge badge-primary">{{ $produkNonForming->shift->shift ?? '-' }}</span>
                                </div>
                                <div class="col-md-3">
                                    <strong>Produk:</strong><br>
                                    <span class="badge badge-success">{{ $produkNonForming->produk->nama_produk ?? '-' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('produk-non-forming.index') }}" class="btn btn-secondary btn-sm">
                        ...
                        <div class="card-body">
                            @php
                                $logs = \App\Models\ProdukNonFormingLog::where('produk_non_forming_uuid', $produkNonForming->uuid)
                                    ->with('user')
                                                ...
                                                <td>
                                                    @if($log->field_yang_diubah && count($log->field_yang_diubah) > 0)
                                                        @foreach($log->field_yang_diubah as $field)
                                                            @php
                                                                $fieldNames = \App\Models\ProdukNonFormingLog::getFieldNames();
                                                                $namaField = $fieldNames[$field] ?? $field;
                                                            @endphp
                                                            <span class="text-primary">{{ $namaField }}</span>
