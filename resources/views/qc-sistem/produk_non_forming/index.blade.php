...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Produk Non Forming</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="">Home</a></li>
                        <li class="breadcrumb-item active">Produk Non Forming</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Data Produk Non Forming</h3>
                            <div class="card-tools">
                                <button type="button" class="btn btn-danger btn-sm mr-2" data-toggle="modal" data-target="#exportPdfModal">
                                    ...
                                    @if(auth()->user()->hasPermissionTo('create-produk-non-forming'))
                                <a href="{{ route('produk-non-forming.create') }}" class="btn btn-primary btn-sm">
                            ...
                            <div class="table-responsive">
                                ...
                                <div class="row mb-3 mt-3">
                                    <div class="col-md-4 offset-md-8">
                                        <form action="{{ route('produk-non-forming.index') }}" method="GET">
                                            <div class="input-group">
                                                ...
                                                <div class="input-group-append">
                                                    ...
                                                    @if(request('search'))
                                                        <a href="{{ route('produk-non-forming.index') }}" class="btn btn-outline-danger" title="Clear Search">
                                ...
                                <table class="table text-center table-bordered table-striped" style="white-space: nowrap;">
                                    ...
                                    <tbody>
                                        @forelse($produkNonFormings as $index => $item)
                                            <tr>
                                                <td>{{ $produkNonFormings->firstItem() + $index }}</td>
                                              <td>
                                            ...
                                            <td>
                                                ...
                                                <!-- <td>
                                                    <span class="badge badge-{{ $item->kemasan_primer == 1 ? 'success' : ($item->kemasan_primer == 2 ? 'warning' : 'danger') }}">
                                                        {{ $item->getKemasanPrimerText() }}
                                                    </span>
                                                </td>
                                                <td>
                                                    <span class="badge badge-{{ $item->kemasan_sekunder == 1 ? 'success' : ($item->kemasan_sekunder == 2 ? 'warning' : 'danger') }}">
                                                        {{ $item->getKemasanSekunderText() }}
                                                    </span>
                                                </td> -->
                                                <!-- <td>
                                                    <span class="badge badge-{{ $item->mesin == 1 ? 'success' : ($item->mesin == 2 ? 'warning' : 'danger') }}">
                                                        {{ $item->getMesinText() }}
                                                    </span>
                                                </td> -->
                                                <!-- <td>
                                                    @if($item->verifikasi_qc == 'approved')
                                                        <span class="badge badge-success">Approved</span>
                                                    @elseif($item->verifikasi_qc == 'rejected')
                                                ...
                                                <td class="text-center">
                                                    <div class="btn-vertical">
                                                        <!-- CRUD Buttons -->
                                                        <!-- <div class="mb-1">
                                                            <a href="{{ route('produk-non-forming.show', $item->uuid) }}" class="btn btn-info btn-sm" title="Detail">
...
@if(auth()->user()->hasPermissionTo('edit-produk-non-forming'))
                                                                <a href="{{ route('produk-non-forming.edit', $item->uuid) }}" class="btn btn-warning btn-sm" title="Edit">
                                                                    <i class="fas fa-edit"></i>
                                                                </a>
                                                           @endif
 @endif
@if(auth()->user()->hasPermissionTo('view-produk-non-forming'))
                                                            <a href="{{ route('produk-non-forming.logs', $item->uuid) }}" class="btn btn-secondary btn-sm" title="Lihat Riwayat">
                                                                <i class="fas fa-history"></i>
                                                            </a>
                                                            @endif
@if(Auth::user()->role == 'superadmin' || Auth::user()->role == 'admin')
@if(auth()->user()->hasPermissionTo('delete-produk-non-forming'))
                                                                <form action="{{ route('produk-non-forming.destroy', $item->uuid) }}" method="POST" style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                                                                    @csrf
                                                                    @method('DELETE')
 ...
 @endif
                                                        ...
                                                        <x-action-buttons :item="$item" route-prefix="produk-non-forming" />

                                                        <!-- Approval Buttons -->
                                                        @php
                                                            $userRole = auth()->user()->id_role ?? null;
                                                            $hasApprovalColumns = true; // Assume approval columns exist for produk_non_forming
                                                        @endphp
                            ...
                            <div class="d-flex justify-content-center mt-4">
                                {{ $produkNonFormings->appends(request()->query())->links('pagination::bootstrap-4') }}
                            </div>
                        </div>
                        <!-- /.card-body -->
                    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exportPdfModalLabel">Cetak PDF Produk Non Forming Berdasarkan Filter</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="bulkExportForm" action="{{ route('produk-non-forming.bulk-export-pdf') }}" method="POST" data-bulk-form="true">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="id_produk" name="produk">
                            ...
                            @foreach($cachedProduk->where('status_bahan', 'non-forming') as $produk)
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="bulkKodeForm" name="kode_form" value="QF 21/00" placeholder="Masukkan kode form" required>
                    </div>
                </form>
            </div>
...
$(document).ready(function() {
    ...
    $('.approve-btn').click(function() {
        ...
        if (confirm(`Apakah Anda yakin ingin menyetujui data ini sebagai ${typeNames[type]}?`)) {
            ...
            $.ajax({
                url: '{{ route("produk-non-forming.approve", ":id") }}'.replace(':id', id),
                method: 'POST',
                data: {
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        button.removeClass('btn-outline-success btn-outline-primary btn-outline-info')
                              .addClass('btn-success btn-primary btn-info'.split(' ')[type === 'qc' ? 0 : type === 'produksi' ? 1 : 2])
                              .prop('disabled', true)
                              .html('<i class="fas fa-check-circle"></i> ' + typeNames[type]);
                        ...
                        button.prop('disabled', false)
                              .html('<i class="fas fa-check"></i> ' + typeNames[type]);
                ...
                error: function(xhr) {
                    ...
                    const errorMessage = response?.message || 'Terjadi kesalahan saat melakukan approval.';
