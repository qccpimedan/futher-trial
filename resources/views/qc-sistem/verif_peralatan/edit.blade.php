...
@section('title', 'Edit Verifikasi Kebersihan Mesin & Peralatan')
    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Edit Verifikasi Kebersihan Ruangan Mesin & Peralatan</h1>
                    </div>
                    <div class="col-sm-6">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            ...
                            <form action="{{ route('verif-peralatan.update', $item->uuid) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="card-body">
                                    @if($errors->any())
                                    ...
                                    <div class="row">
                                        @if(auth()->user()->role === 'superadmin')
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label>Plan</label>
                                                    <input type="text" class="form-control" value="{{ optional($plans->firstWhere('id', $selectedPlanId))->nama_plan ?? ($item->plan->nama_plan ?? '-') }}" readonly>
                                                </div>
                                            </div>
                                        @endif
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="id_shift" id="id_shift" class="form-control @error('id_shift') is-invalid @enderror" required>
                                                    ...
                                                    @foreach($shifts as $shift)
                                                        <option value="{{ $shift->id }}" {{ (old('id_shift', $item->id_shift) == $shift->id) ? 'selected' : '' }}>
                                                            {{ $shift->shift }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="date" name="tanggal" id="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', \Carbon\Carbon::parse($item->tanggal)->format('Y-m-d')) }}" required>
                                                @error('tanggal')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="time" name="jam" id="jam" class="form-control @error('jam') is-invalid @enderror" value="{{ old('jam', $item->jam ? \Carbon\Carbon::parse($item->jam)->format('H:i') : now()->format('H:i')) }}">
                                                @error('jam')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                                        ...
                                                        <table class="table table-bordered table-striped" style="white-space:nowrap;">
                                                            ...
                                                            <tbody>
                                                                @foreach($areaMesins as $mesin)
                                                                    @php
                                                                        $row = $existing->get($mesin->id);
                                                                        $oldVerif = old("details.{$mesin->id}.verifikasi");
                                                                        $verifVal = ($oldVerif === null) ? (int) (bool) ($row?->verifikasi) : (int) (bool) $oldVerif;
                                                                    @endphp
                                                                    <tr>
                                                                        <td class="text-center">
                                                                            @php
                                                                                $verifSelectVal = (string) $verifVal;
                                                                            @endphp
                                                                            <select name="details[{{ $mesin->id }}][verifikasi]" class="form-control form-control-sm verifikasi-select" data-area-id="{{ $area->id }}">
                                                                                <option value="1" {{ $verifSelectVal === '1' ? 'selected' : '' }}>OK</option>
                                                                                <option value="0" {{ $verifSelectVal === '0' ? 'selected' : '' }}>Tidak OK</option>
                                                                            </select>
                                                                        </td>
                                                                        <td>{{ $mesin->nama_mesin }}</td>
                                                                        <td>
                                                                            <div class="verif-only-notok" data-mesin-id="{{ $mesin->id }}">
                                                                                <textarea name="details[{{ $mesin->id }}][keterangan]" class="form-control" rows="2">{{ old("details.{$mesin->id}.keterangan", $row?->keterangan) }}</textarea>
                                                                            </div>
                                                                        </td>
                                                                        <td>
                                                                            <div class="verif-only-notok" data-mesin-id="{{ $mesin->id }}">
                                                                                <textarea name="details[{{ $mesin->id }}][tindakan_koreksi]" class="form-control" rows="2">{{ old("details.{$mesin->id}.tindakan_koreksi", $row?->tindakan_koreksi) }}</textarea>
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                                @endforeach
