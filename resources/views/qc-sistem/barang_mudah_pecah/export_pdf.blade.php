...
<head>
    <meta charset="utf-8">
    <title>Verifikasi Proses Produksi - Barang Mudah Pecah</title>
    <style>
        body {
...
<body>
    ...
    <div class="title">VERIFIKASI KONDISI BARANG MUDAH PECAH</div>

    @if($data->count() > 0)
        @php
            $firstItem = $data->first();
            $groupedByArea = $data->groupBy(function ($item) {
                return $item->area->area ?? 'Area Tidak Diketahui';
            });
            $no = 1;
        @endphp
        ...
        <table class="main-table">
            ...
            <tbody>
                @foreach($groupedByArea as $areaName => $items)
                    <tr>
                        <td colspan="4" style="text-align: left; font-weight: bold; background-color: #e9ecef;">
                            {{ $areaName }}
                        </td>
                    </tr>
                    @foreach($items as $item)
                        <tr>
                            ...
                            <td>
                                ...
                                @elseif($item->kondisi == 'Tidak OK')
                                    <span class="status-not-ok">Tidak OK</span>
                                @else
                                    {{ $item->kondisi ?? '-' }}
                                @endif
                            </td>
